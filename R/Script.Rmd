...
for (k in 1:nb_windows){
  ...
  for (l in 1:nb_windows){
    if (l != k){
      diff_beta <- abs(list_half_beta[[k]] - list_half_beta[[l]])
      #diff_beta <- sqrt(list_half_beta[[k]]^2 + list_half_beta[[l]]^2)
      m <- m + 1
      list_diff[[m]] <- diff_beta
  ...
  tot_diff[[k]] <- Reduce("+", list_diff)
}

tot_sum <- Reduce("+", tot_diff)
...
data0 <- Reduce("+", tot_diff)/nb_windows
...
for (i in 1:25){
  ...
  data <- paste0("data", i)
  hist(x = tot_diff[[i]], col = "cyan", main = paste0("Window",":",i), xlim = c(0,200), ylim = c(0,1e6))
  hist(x = data0, col = add.alpha("purple",.2), add = TRUE)
}
...
for (n in 1:nb_windows) {
  res <- ks.test(x = data0, y = tot_diff[[n]])
  results_ks[[n]] <- res$statistic[[1]]
  print(n)
}
...
```{r}
essai <- ks.test(x = data0, y = tot_diff[[31]])
print(essai$statistic[[1]])
```

```{r}
plot(unlist(results_ks))
hist(unlist(results_ks))
```

### Detect if a window is significantly further from the mean of all windows
```{r}
grubb_res <- grubbs.test(unlist(results_ks), type = 10)
if (grubb_res$p.value < 0.05){
  w_interest <- which.max(results_ks)
  print(w_interest)
}
```
